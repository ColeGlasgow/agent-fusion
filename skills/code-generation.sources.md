...
- Iteration 4 added two examples chosen for highest real-world leverage: a hallucinated OpenAI SDK call (covers Rule 1, the most common AI failure mode and previously demonstration-free) and a fake retry test (covers Rule 5, the most insidious testing failure mode — tests that pass whether or not the change exists). Both examples cover rules that had only rule-text before; the four examples now span Rules 1, 2, 5, 6, 8, 9.
- PR #11's audit removed Rule 6's `TODO` escape hatch for security-sensitive boundaries; unresolved boundary work now belongs in `Assumptions and uncertainty`.
